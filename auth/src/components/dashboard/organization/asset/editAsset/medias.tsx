...
import { FiTrash2, FiStar, FiEdit2, FiInfo } from 'react-icons/fi';
...
const AssetImageManager: React.FC<AssetImageManagerProps> = ({
  assetID,
  organizationID,
  maxImages = 3,
  initialAssetMedias,
  ...
  const [editingLegend, setEditingLegend] = React.useState<string | null>(null);
  const [newLegend, setNewLegend] = React.useState('');

  // Filtrer les médias non supprimés pour l'affichage
  const activeMedias = assetMedias.filter(media => !media.toDelete);

  const handleFileUpload = async (file: File) => {
    if (activeMedias.length >= maxImages) {
      alert(`Vous ne pouvez avoir plus de ${maxImages} images par asset`);
      return;
    }
  ...
  const handleDeleteImage = async (assetMediaID: string) => {
  ...
  const handleSetMainImage = async (assetMediaID: string) => {
    try {
      setIsLoading(true);
      
      const targetIndex = activeMedias.findIndex(m => m.assetMediaID === assetMediaID);
      if (targetIndex > 0) {
        ...
        reorderAssetMedias(reorderedMedias);
      }
    } catch (error) {
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    reorderAssetMedias(reorderedItems);
  };

  const handleEditLegend = async (assetMediaID: string) => {
    try {
      setIsLoading(true);
      updateAssetMediaLegend(assetMediaID, newLegend);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-medium">Images de l'asset ({activeMedias.length}/{maxImages})</h3>
      </div>

      {/* Information sur le format */}
      <div className="flex items-center gap-2 text-sm text-blue-600 bg-blue-50 p-3 rounded-lg">
        <FiInfo size={16} />
        <span>Format recommandé : <strong>400x300 pixels</strong> (ratio 1.33:1) pour un affichage optimal</span>
      </div>
          ...
          {(provided) => (
            ...
            >
              ...
              {activeMedias
                ...
                .slice(0, maxImages) // Limiter à maxImages
                .map((media, index) => (
                  <Draggable
                    key={media.assetMediaID}
                    draggableId={media.assetMediaID}
                    index={index}
                  >
                    {(provided, snapshot) => (
                      <div
                      ...
                      >
                        ...
                        <div className="relative" style={{ aspectRatio: '400/300' }}>
                          ...
                          <div 
                            ...
                            onClick={() => handleSetMainImage(media.assetMediaID)}
                          ...
                          <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-200 flex items-center justify-center">
                            <div className="opacity-0 group-hover:opacity-100 flex space-x-2">
                              <button
                                onClick={() => {
                                  setEditingLegend(media.assetMediaID);
                              ...
                              <button
                                onClick={() => handleDeleteImage(media.assetMediaID)}
                                disabled={isLoading}
                        ...
                        {/* Legend */}
                        <div className="p-3">
                          {editingLegend === media.assetMediaID ? (
                            <div className="space-y-2">
                              ...
                              <div className="flex space-x-2">
                                <Button
                                  onClick={() => handleEditLegend(media.assetMediaID)}
                                  disabled={isLoading}
                                  className="text-xs"
                                >
                                  Sauvegarder
